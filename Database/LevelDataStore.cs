...
namespace DevSpace.Database {
	public class LevelDataStore : IDataStore<ITag> {
		...
		public async Task<ITag> Get( int Id ) {
			try {
				using( SqlConnection connection = new SqlConnection( Settings.ConnectionString ) ) {
					...
					using( SqlCommand command = new SqlCommand( "SELECT * FROM Levels WHERE Id = @Id", connection ) ) {
		...
		public async Task<IList<ITag>> GetAll() {
			...
			using( SqlConnection connection = new SqlConnection( Settings.ConnectionString ) ) {
				...
				using( SqlCommand command = new SqlCommand( "SELECT * FROM Levels", connection ) ) {

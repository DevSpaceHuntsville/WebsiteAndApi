...
namespace DevSpace.Api.Controllers {
	...
	public class UserController : ApiController {
		...
		private async Task<string> CreateJsonUserArray( IList<IUser> Users ) {
			...
			IList<ISession> SessionList = ( await SessionsDS.GetAll() ).Where( ses => ses.EventId == 2020 ).Where( ses => ses.Accepted ?? false ).ToList();
		...
		public async Task<HttpResponseMessage> Get( int Id ) {
			if( Thread.CurrentPrincipal.Identity.IsAuthenticated ) {
				...
				try {
					Database.SessionDataStore SessionsDS = new Database.SessionDataStore();
					IList<ISession> SessionList = ( await SessionsDS.GetAll() ).Where( ses => ses.UserId == Id ).Where( ses => ses.Accepted ?? false ).Where( ses => ses.EventId == 2020 ).ToList();
